...
def extract_usecase(sheet_ranges):
  ...
  relobj.name                   = cell(sheet_ranges, 15, column) or 'None'
  usecase.RelatedObjective.append(relobj)
  bcase                         = IEC62559.Ref_BusinessCase()
  bcase.mRID                    = cell(sheet_ranges, 16, column) or 'None'
  usecase.BusinessCase.append(bcase)
  narrative                     = IEC62559.Narrative()
  narrative.shortDescription    = cell(sheet_ranges, 18, column)
  narrative.completeDescription = cell(sheet_ranges, 19, column)
  usecase.Narrative             = narrative
  refusecase                    = IEC62559.Ref_UseCase()
  refusecase.mRID               = cell(sheet_ranges, 29, column) or 'None'
  ...
  drawing                       = extract_drawing(sheet_ranges)
  if drawing:
